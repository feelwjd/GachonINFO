...
var { mapSubject } = require('./mapping');
...
router.post('/send', function (req, res, next) {
    ...
    con.query(sql, async (err, result) => {
        if (err) {
            logger.info(`B_LNK_SD SE PROC_STEP:1 SUCC_CNT:0 FAIL_CNT:1`);
            logger.error(`B_LNK_SD SE ERROR_CODE:EAC001`);
            console.log(err);
        }
        else {
            if (result[0] === undefined) {
                logger.info(`B_LNK_SD SE PROC_STEP:2 SUCC_CNT:1 FAIL_CNT:1`);
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    jsonObj.class_time = String(result[i].class_course);
                    jsonObj.grade = '미처리';
                ...
                logger.info(`B_LNK_SD SE PROC_STEP:2 SUCC_CNT:2 FAIL_CNT:0`);
                console.log(jsonArr);
...
router.post('/identify', function (req, res, next) {
    ...
    var sql_identifyUniv = `
                    SELECT
                        student_name, student_id, department_name
                    FROM
                        b_student
                    WHERE
                        student_name=?
    ...
    con.query(sql_identifyUniv, name, async (err, result) => {
        ...
        if (err) {
            ...
            if (result[0] === undefined) {
                ...
                jsonObj.department_name = result[0].department_name;
                logger.info(`B_LNK_SD SE PROC_STEP:2 SUCC_CNT:2 FAIL_CNT:0`);
                console.log(jsonObj);
